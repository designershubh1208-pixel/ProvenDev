...
import { Plus, Cpu, ShieldCheck, Clock, MessageSquare, X, Github, ExternalLink, Link } from 'lucide-react'; // Added Link
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-700 p-2 relative">
            <div className="flex justify-between items-end px-2">
                ...
                <button onClick={() => setIsModalOpen(true)} className="bg-indigo-600 hover:bg-indigo-700 text-white px-8 py-4 rounded-2xl font-bold shadow-lg flex items-center gap-2"><Plus size={20} /> Add New Skill</button>
            </div>
                            ...
                            <tr key={skill.id} className="hover:bg-slate-50/50 transition-colors">
                                ...
                                <td className="py-8 px-4 text-center">
                                    <div className="flex flex-col items-center gap-2">
                                        <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl border font-black text-[10px] uppercase tracking-widest ${skill.status === 'Verified' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-amber-50 text-amber-600 border-amber-100'}`}>{skill.status === 'Verified' ? <ShieldCheck size={14} /> : <Clock size={14} />}{skill.status || "Pending"}</div>
                                        {skill.feedback && (<div className="bg-indigo-50/50 px-3 py-1.5 rounded-lg border border-indigo-100 text-[11px] font-bold text-indigo-500 flex items-center gap-2"><MessageSquare size={12} /> {skill.feedback}</div>)}

                                        {/* BLOCKCHAIN PROOF BUTTON */}
                                        {skill.blockchainHash && (
                                            <a href={`https://polygonscan.com/tx/${skill.blockchainHash}`} target="_blank" rel="noreferrer" className="flex items-center gap-1 text-[10px] font-black text-indigo-600 bg-indigo-50 px-3 py-1.5 rounded-lg hover:bg-indigo-100 transition-colors">
                                                <Link size={12} /> View On-Chain Proof
                                            </a>
                                        )}
                                    </div>
                                </td>
